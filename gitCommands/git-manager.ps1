...
function Invoke-CloneOne {
    param([string]$RepoUrl, [string]$SuggestedName)
    ...
    Write-Info "Clonando para '$targetPath'..."
    ...
    $out | Where-Object { $_ -notmatch '^\s*$' } | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }

    $addResp = Read-Host '  Adicionar ao workspace agora? (S/n)'
    if ($addResp -notmatch '^[nN]$') {
        Add-WorkspaceFolder -FolderPath $targetPath -FolderName $folderName | Out-Null
    }
    else {
        Write-Info "Repositorio clonado em '$targetPath' mas NAO adicionado ao workspace."
    }
}

function Invoke-CloneAndAddToWorkspace {
    Write-Header 'BAIXAR REPOSITORIO(S) E ADICIONAR AO WORKSPACE'
    Write-Bold "Workspace : $WorkspaceFile"
    Write-Bold "Pasta base: $WorkspaceRoot"
    Write-Host ''

    do {
        $repoUrl = Read-Host '  URL do repositorio Git (Enter = encerrar)'
        if ([string]::IsNullOrWhiteSpace($repoUrl)) {
            Write-Host '  Encerrado.'
            return
        }

        Invoke-CloneOne -RepoUrl $repoUrl
        Write-Host ''

        $more = Read-Host '  Clonar outro repositorio? (s/N)'
    } while ($more -match '^[sS]$')
}
